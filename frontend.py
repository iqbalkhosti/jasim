...
class CatalogApp:
    ...
    def login_screen(self):
        ...
        tk.Button(frame, text="Login", command=lambda: self.main_menu() if username_entry.get() == "username" and password_entry.get() == "password" else messagebox.showerror("Login Failed", "Invalid username or password"), bg="#4682B4", fg="white").pack(pady=10)
    ...
    def main_menu(self):
        ...
        dropdown = ttk.Combobox(frame, textvariable=self.selected_option, values=["Display Catalog", "View Item Details", "Add Entry", "Remove Entry", "Exit"], state="readonly")
    ...
    def display_results(self, results):
        ...
        for item in results:
            tk.Button(frame, text=f"{item['ID']}: {item['Make']} {item['Model']}", command=lambda i=item: self.display_item_details(i), bg="white", relief=tk.RIDGE).pack(fill=tk.X, padx=10, pady=2)
    ...
    def view_item(self):
        ...
        self.display_item_details(item) if item else messagebox.showerror("Error", "Item not found")
